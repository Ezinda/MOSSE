...
Begin VB.Form frmrecibointrumento 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Asignacion de Recibos de Cobro Como instrumento de Pago"
   ClientHeight    =   6105
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   7860
   ...
   Begin vbskpro.Skinner Skinner1 
      ...
      LcK2            =   $"frmrecibointrumento.frx":0000
      AmbientB        =   ";<=>?7B:><7=<A<7CC;@"
   ...
   End
End
Attribute VB_Name = "frmrecibointrumento"
...
frmordenintrumento.Left = 600
  ...
  datinstru.RecordSource = "select consultarecibosinc.* from consultarecibosinc where empresa = " & login.empresaact & " and importe <> 0 and anulado <> 'S' order by nomcliente, nrorden"
  datinstru.Refresh
  
  If datinstru.Recordset.EOF = True Then
        mensa = MsgBox("No tiene Recibos sin Asignar", vbInformation, "Mensaje")
        Unload Me
        Exit Sub
  End If
...
Do While Not datinstru.Recordset.EOF
  ...
  grilla.Col = 3
  grilla.Text = datinstru.Recordset.Fields("nomcliente")
  grilla.Col = 4
...
Private Sub grilla_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        KeyAscii = 0
        grilla.Col = 1
        frmrecibos1.Text5(0).Text = "Recibo " + grilla.Text
        frmrecibos1.ordeninstu = grilla.Text
        grilla.Col = 4
        If grilla.Text < 0 Then grilla.Text = grilla.Text * -1
        frmrecibos1.importemask.Text = grilla.Text
        frmrecibos1.importeord = grilla.Text
        grilla.Col = 2
     Rem    frmrecibos1.MaskEdBox1.Text = grilla.Text
        frmrecibos1.Text5(1).Text = Cuenta(grilla.Row)

        Unload Me
        frmrecibos1.importemask.SetFocus
    End If
