...
Begin VB.Form frmreciboanula 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Anular Recibos de Cobranza"
   ClientHeight    =   2205
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4200
   Icon            =   "frmreciboanula.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   MinButton       =   0   'False
   ...
   Begin MSDataGridLib.DataGrid DataGrid4 
      Bindings        =   "frmreciboanula.frx":0442
      Height          =   255
      Left            =   1320
      TabIndex        =   10
      Top             =   1680
   ...
   Begin MSDataGridLib.DataGrid DataGrid3 
      Bindings        =   "frmreciboanula.frx":045A
      Height          =   255
      Left            =   0
      TabIndex        =   9
      Top             =   1680
   ...
   Begin MSDataGridLib.DataGrid DataGrid2 
      Bindings        =   "frmreciboanula.frx":0477
      Height          =   255
      Left            =   120
      TabIndex        =   7
      Top             =   960
   ...
   Begin MSDataGridLib.DataGrid DataGrid1 
      Bindings        =   "frmreciboanula.frx":0494
      Height          =   255
      Left            =   1080
      TabIndex        =   6
      Top             =   960
   ...
   Begin VB.CommandButton Command4 
      ...
      Height          =   735
      Left            =   480
      Picture         =   "frmreciboanula.frx":04AD
      Style           =   1  'Graphical
      TabIndex        =   5
      Top             =   1200
      UseMaskColor    =   -1  'True
   ...
   Begin MSDataListLib.DataCombo DataCombo1 
      Bindings        =   "frmreciboanula.frx":05AF
      Height          =   420
      Left            =   240
      TabIndex        =   0
      Top             =   480
   ...
   Begin VB.CommandButton salir 
      ...
      Height          =   735
      Left            =   2280
      Picture         =   "frmreciboanula.frx":05CC
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   1200
      UseMaskColor    =   -1  'True
   ...
   Begin MSMask.MaskEdBox totalabonan 
      ...
      PromptChar      =   "_"
   End
   Begin MSAdodcLib.Adodc datrecibocobro 
      Height          =   330
      Left            =   480
      Top             =   1320
      Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "frmreciboanula"
...
Dim nroreciboanul As Double


Private Sub anular_Click()
On Error Resume Next
        
        DataGrid2.Bookmark = DataCombo1.SelectedItem
        If DataGrid2.Columns(6).Text = "" Then DataGrid2.Columns(6).Text = 0
        databonan.RecordSource = "select recibocobroabonan.* from recibocobroabonan where empresa = " & login.empresaact & " and inicioper = '" & login.iper & "' and nrorden = '" & DataCombo1.Text & "'"
        databonan.Refresh

        
        If databonan.Recordset.EOF = True Then
            mensa = MsgBox("Este Recibo pertenece a un Periodo Contable Anterior, ingrese al periodo correspondiente e intente nuevamente", vbCritical, "Error")
            Exit Sub
        End If
              
        databonan.Recordset.MoveFirst
        
        datbuscarecibo.RecordSource = "select recibocobro.* from recibocobro WHERE recibocobro.empresa = " & login.empresaact & " AND nrorden = '" & DataCombo1.Text & "' "
        datbuscarecibo.Refresh
        
        datmaestro.RecordSource = "select [Maestro Asientos].* from [Maestro Asientos] where empresa = " & login.empresaact & " and perinicial = '" & login.iper & "' and nroasiento = " & datbuscarecibo.Recordset.Fields("idasiento") & " "
        datmaestro.Refresh
                     
        mensa = MsgBox("Esta segura de anular este recibo", vbYesNo, "Atención")
        If mensa = vbYes Then
        If datmaestro.Recordset.EOF = True Then GoTo sigue
            idmaster = datmaestro.Recordset.Fields("idmasterasientos")
            datmaestro.Recordset.Delete adAffectCurrent
sigue:
            datrecibocobro.RecordSource = "select recibocobro.* from recibocobro where empresa = " & login.empresaact & " and nrorden = '" & DataCombo1.Text & "'"
            datrecibocobro.Refresh
            datrecibocobro.Recordset.Fields("anulado") = "S"
            datrecibocobro.Recordset.Fields("idasiento") = 0
            datrecibocobro.Recordset.UpdateBatch adAffectCurrent
            
            Do While Not databonan.Recordset.EOF
                cli = databonan.Recordset.Fields("nomcliente")
                tip = Left(databonan.Recordset.Fields("comprobante"), 3)
                num = Right(databonan.Recordset.Fields("comprobante"), 13)
                datlibroventas.RecordSource = "select libroventas.* from libroventas where empresa = " & login.empresaact & " and cliente = '" & cli & "' and tipocompr = '" & tip & "' and numcompr = '" & num & "'  "
                datlibroventas.Refresh
                datlibroventas.Recordset.Fields("saldo") = datlibroventas.Recordset.Fields("saldo") + databonan.Recordset.Fields("importe")
                datlibroventas.Recordset.UpdateBatch adAffectCurrent
                databonan.Recordset.MoveNext
            Loop
                ...
                Inicio.datauditoria.Recordset.Fields("ventana") = "Anular Recibos de Cobranza"
                Inicio.datauditoria.Recordset.Fields("accion") = "Anulacion Recibo: " + DataCombo1.Text
                ...
                mensa = MsgBox("RECIBO ANULADO", vbInformation, "Información")
        End If
        
Exit Sub
fuera:
    mensa = MsgBox("Selecciones la Orden y anule", vbCritical, "Error")
...
With CrystalReporte
  ...
  If Left(DataCombo1.Text, 4) <> "LCM-" Then
...
frmreciboanula.Top = 0
frmreciboanula.Left = 0
...
datrecibocobro.ConnectionString = login.conexiontotal
